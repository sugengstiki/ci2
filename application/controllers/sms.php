		...
		*/
		...
		foreach ($penerima as $hp){
			...
			if(is_array($hp)){				
				...
				$data['sms_message']= strlen($hp['C'])>0?$hp['C']:$data['sms_message'];// . date('Y-m-d G:i:s');
				if (empty($data['sms_sendto'])) continue;
			...
			'TextDecoded'=> $data['sms_message'],
			//'SendingDateTime' => date('Y-m-d H:i:s',strtotime('+4 seconds')),
			'CreatorID'=>$data['sms_pengirim']		
			);		
